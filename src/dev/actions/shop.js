...
export function removeTransaction(transaction, user){
    return dispatch => {
        ...
        let KURKKUURL = typeof(KURKKUSERVER) === "undefined" ? 'http://localhost:3000/removeTransaction' : KURKKUSERVER + '/removeTransaction'
...
function _completePendingPayment(dispatch, pendingTrxId){
    ...
    let KURKKUURL = typeof(KURKKUSERVER) === "undefined" ? 'http://localhost:3000/approveincomplete' : KURKKUSERVER + '/approveincomplete'
    firebase.auth().currentUser.getToken(true)
        .then(idToken => {
            return axios.post(KURKKUURL, {
                current_user: idToken,
...
export function buyDelayed(pendingTrxId) {
    return dispatch => {
      _showLoadingScreen(dispatch, "Lähetetään lasku ostosta.")
        let KURKKUURL = typeof(KURKKUSERVER) === "undefined" ? 'http://localhost:3000/notifydelayed' : KURKKUSERVER + '/notifydelayed'
        firebase.auth().currentUser.getToken(true)
            .then(idToken => {
                return axios.post(KURKKUURL, {
                    current_user: idToken,
...
export function finishPayTrailTransaction(query){
        ...
        let KURKKUURL = typeof(KURKKUSERVER) === "undefined" ? 'http://localhost:3000/completepaytrail' : KURKKUSERVER + '/completepaytrail'
        firebase.auth().currentUser.getToken(true)
            .then(idToken => {
                return axios.post(KURKKUURL, {
                    current_user: idToken,
...
export function getAuthCode(_authcode) {
    return dispatch => {
        ...
        let KURKKUURL = typeof(KURKKUSERVER) === "undefined" ? 'http://localhost:3000/paytrailauthcode' : KURKKUSERVER + '/paytrailauthcode'
        firebase.auth().currentUser.getToken(true)
            .then(idToken => {
                return axios.post(KURKKUURL, {
                    current_user: idToken,
...
function _cancelPaytrailPayment(dispatch, pendingTrxId, resetShop = true) {
    ...
    let KURKKUURL = typeof(KURKKUSERVER) === "undefined" ? 'http://localhost:3000/cnacelpaytrailtransaction' : KURKKUSERVER + '/cancelpaytrailtransaction'
...
export function initializePayTrailTransaction(shopItem, type) {
    return dispatch => {
        ...
        let KURKKUURL = typeof(KURKKUSERVER) === "undefined" ? 'http://localhost:3000/initializepaytrailtransaction' : KURKKUSERVER + '/initializepaytrailtransaction'
...
export function initializeDelayedTransaction(shopItem, type) {
    return dispatch => {
        _showLoadingScreen(dispatch, "Alustetaan maksutapahtuma")
        let KURKKUURL = typeof(KURKKUSERVER) === "undefined" ? 'http://localhost:3000/initializedelayedtransaction' : KURKKUSERVER + '/initializedelayedtransaction'
...
export function executeCashPurchase(forUsr, itemKey, type) {
    return dispatch => {
        ...
        let KURKKUURL = typeof(KURKKUSERVER) === "undefined" ? 'http://localhost:3000/cashbuy' : KURKKUSERVER + '/cashbuy'
...
/*export function getClientTokenFromBraintree() {
    return dispatch => {
        ...
        let KURKKUURL = typeof(KURKKUSERVER) === "undefined" ? 'http://localhost:3000/clientToken' : KURKKUSERVER + '/clientToken'
        firebase.auth().currentUser.getToken(true)
            .then(idToken => {
                return axios.get(KURKKUURL + '?token=' + idToken)
            })
            .then(response => {
...
export function doPurchaseTransaction(shopItem) {
    return dispatch => {
        ...
        let KURKKUURL = typeof(KURKKUSERVER) === "undefined" ? 'http://localhost:3000/checkout' : KURKKUSERVER + '/checkout'
