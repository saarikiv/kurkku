...
export function postCancellation(item, txRef, slotInfo) {
    var KURKKUURL = typeof(KURKKUSERVER) === "undefined" ? 'http://localhost:3000/cancelSlot' : KURKKUSERVER + '/cancelSlot'
    return dispatch => {
        _showLoadingScreen(dispatch, "Perutaan KURKKUta")
        let now = new Date();
            ...
            axios.post(
                ...
                .then(response => {
                  dispatch({
                    ...
                    _hideLoadingScreen(dispatch, "KURKKU peruttu", true)
                })
                .catch(error => {
                  dispatch({
                    type: CANCEL_ERROR,
...
export function postReservation(forward, slotInfo) {
    var KURKKUURL = typeof(KURKKUSERVER) === "undefined" ? 'http://localhost:3000/reserveSlot' : KURKKUSERVER + '/reserveSlot'
    return dispatch => {
            ...
            axios.post(
                ...
                .then(response => {
                  dispatch({
                    ...
                    _hideLoadingScreen(dispatch, "KURKKU onnistui", true)
                })
                .catch(error => {
                  dispatch({
                    type: BOOKING_ERROR,
